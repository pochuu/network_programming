...
int main(int argc, char **argv) {
  ...
  snl.nl_pid = getpid();
  snl.nl_groups = RTMGRP_IPV4_ROUTE | RTMGRP_NOTIFY; //grupy ktore chcemy monitorowac 
  //sudo ip route del 20.0.0.0/8 via 150.254.32.130 usuwamy z routingu
  ...
  while(1) {
    ...
    do {
      ...
      ptr += rclen;
      nllen += rclen;
    } while(nlp->nlmsg_type == NLMSG_DONE); //odbieranie komunikatow od jadra
    //ip route show table 253
    //echo 1 > /proc/sys/net/ipv4/ip_forward 
    //RTM_DELROUTE
    //sudo ./rtset 11.0.0.0 8 150.254.32.130^C
    ...
    for(;NLMSG_OK(nlp, nllen); nlp = NLMSG_NEXT(nlp, nllen)) {
      ...
      if (nlp->nlmsg_type == RTM_DELROUTE) //sprawdzamy czy dodanie wpisu czy usuniecie
        printf("[DEL] ");
